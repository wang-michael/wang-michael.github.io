...
当我们通过ByteBuffer的allocateDirect()方法来分配内存时，方法内部会判断当前分配出去的直接内存区域大小是否达到了设定的阈值，如果达到了，则调用System.gc()方法建议JVM进行一次GC，由于每个分配出去的DirectBuffer对象都关联了一个Cleaner对象，所以如果在GC时DirectBuffer对象是虚可达的，其对应的直接内存就会被回收。这个过程中需要注意几个问题：  
